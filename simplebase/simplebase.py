    ...
    class Collection:
        ...
        def register_hash_index(self,name,key,**kwargs):
            ...
            """if kwargs.get("reindex")==True:
                ...
                self._db[name].insert_many(set,indexing=True)
                    #self[hash(key)]=key_document"""
        ...
        def _insert_index(self,document):
          self._data[document['_id']]=document['value']

          result_keyvalpairs = self._data.items()
          #nparr = np.array(list(result_keyvalpairs))  

          nparr = np.fromiter(result_keyvalpairs,dtype=object)  

          np.save(str(self._path).replace('json','npy'), nparr)
        ...
        def get_data(self):
            ...
            for key,value in self._data.items():
                document = self._db[value['collection']].get(key)
